...
import { FACEBOOK_LEAD_ADS_KINDS } from "../src/index.js";
...
test("the registry names are exactly what the manifest says", () => {
  assert.deepEqual(FACEBOOK_LEAD_ADS_KINDS.map((kind) => kind.name), [
    "@particle-academy/facebook_lead_ads_lead",
    "@particle-academy/facebook_lead_ads_webhook_trigger"
  ]);
});

test("a trigger takes no input port — it starts a run rather than continuing one", () => {
  assert.deepEqual(FACEBOOK_LEAD_ADS_KINDS.find((k) => k.name.endsWith("_webhook_trigger"))?.inputs, []);
});
