/*
 * Facebook Lead Ads — the published npm packages.
 *
...
import { facebookLeadAdsFaker } from "@particle-academy/facebook-lead-ads-js";
import { FACEBOOK_LEAD_ADS_KINDS } from "@particle-academy/facebook-lead-ads-ui";
 ...
 */
const resolved = import.meta.resolve("@particle-academy/facebook-lead-ads-js");
...
for (const { operation, config, expected } of GOLDENS) {
  const faked = facebookLeadAdsFaker(operation, fakeRequest("facebook_lead_ads", operation, config));
...
assert.equal(FACEBOOK_LEAD_ADS_KINDS.length, 2);
for (const kind of FACEBOOK_LEAD_ADS_KINDS) {
...
console.log(`  ok   ui kinds resolve from ${"@particle-academy/facebook-lead-ads-ui"}`);
